...
<xsl:stylesheet xmlns:i18n="http://apache.org/cocoon/i18n/2.1"
  ...
  <!-- The following template should be limited to inslib template structure -->
  <xsl:template priority="10" match="t:title[not(ancestor::t:titleStmt)]">
    <i><xsl:apply-templates/></i>
  </xsl:template>
  
  <!-- The following template should be limited to inslib template structure; move into htm-teibibl.xsl? -->
  <xsl:template priority="10" match="t:ptr[@target]">
    <!-- if you are running this template outside EFES, change the path to the bibliography authority list accordingly -->
    <xsl:variable name="bibliography-al" select="concat('file:',system-property('user.dir'),'/webapps/ROOT/content/xml/authority/bibliography.xml')"/>
    ...
    <xsl:choose>
      <xsl:when test="doc-available($bibliography-al) = fn:true()">
        ...
        <a href="{concat('../concordance/bibliography/', $bibl-ref, '.html')}" target="_blank">
          <xsl:choose>
