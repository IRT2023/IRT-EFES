  ...
  <xsl:template match="response/result">
    ...
    <table class="index tablesorter">
      ...
      <tbody>
        ...
        <xsl:if test="doc[arr[@name='language_code']='xpu']">
          <table class="index tablesorter">
            <xsl:apply-templates select="/aggregation/index_metadata/tei:div/tei:div[@type='headings']" />
            <tbody>
              ...
              <xsl:if test="doc[not(ancestor::aggregation/index_metadata/tei:div[@xml:id=('findspot', 'emperors', 'months', 'epithets')])]"><xsl:apply-templates select="doc[arr[@name='language_code']='xpu']"><xsl:sort select="translate(normalize-unicode(lower-case(translate(., 'ᵓᶜ', '!,')),'NFD'), '&#x0300;&#x0301;&#x0308;&#x0303;&#x0304;&#x0313;&#x0314;&#x0345;&#x0342;' ,'')"/></xsl:apply-templates></xsl:if>
            </tbody>
          </table>
        </xsl:if>
